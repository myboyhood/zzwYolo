...
#include<gaofen2020/RequestVisionPoseSrv.h>
#include<gaofen2020/WaitVisionPoseSrv.h>
...
bool requestVisionCb(gaofen2020::RequestVisionPoseSrv::Request  &req,
                     gaofen2020::RequestVisionPoseSrv::Response &res);


// server client
bool isRequestFromgGaofen = false;
bool targetRegionGood = false;
bool targetDistanceGood = false;
int loopDetectID = 0;
int loopRequestID = 0;
...
int main(int argc, char **argv) {
    ...
    // receive request from gaofen2020Node
    ros::ServiceServer requestServer = nh.advertiseService("requestVision",requestVisionCb);
    ros::ServiceClient provideVisionClient = nh.serviceClient<gaofen2020::WaitVisionPoseSrv>("waitVisionTopic");
    //tf 参数设置
    tf_param_set();
    ...
    while(ros::ok()){
         ...
         if(!targetDetectFlag){
             ...
             msg_drone_pos_vision.pose.orientation.w = -1000;
               ...
               msg_drone_pos_vision.pose.orientation.w = id_name;

               msg_drone_pos_vision.pose.orientation.x = 1;//标志位

               targetDistanceGood = false;
               if(msg_drone_pos_vision.pose.position.x > 0.5 && msg_drone_pos_vision.pose.position.x < 2 &&
                  msg_drone_pos_vision.pose.position.y > -1.0 && msg_drone_pos_vision.pose.position.y <1.0 &&
                  msg_drone_pos_vision.pose.position.z > -0.5 && msg_drone_pos_vision.pose.position.z < 1)
               {
                   targetDistanceGood = true;
               }
         }
        cout<<"x is :"<<msg_drone_pos_vision.pose.position.x<<endl;

        cout<<"y is :"<<msg_drone_pos_vision.pose.position.y<<endl;

        cout<<"z is :"<<msg_drone_pos_vision.pose.position.z<<endl;
        msg_target_pose_from_img_pub.publish(msg_target_pose_from_img);
        drone_pos_vision_pub.publish(msg_drone_pos_vision);
        if(loopRequestID == loopDetectID){
            // if good ,send a request to gaofen2020 updateDrift
            if(targetRegionGood && targetDistanceGood && isRequestFromgGaofen){
                isRequestFromgGaofen = false;
                gaofen2020::WaitVisionPoseSrv srv;
                srv.request.visionPoseDataX = msg_drone_pos_vision.pose.position.x;
                srv.request.visionPoseDataY = msg_drone_pos_vision.pose.position.y;
                srv.request.visionPoseDataZ = msg_drone_pos_vision.pose.position.z;
                if(provideVisionClient.call(srv)){
                    ROS_WARN("provide visionPose success !!!");
                } else{
                    ROS_ERROR("fail to send visionPose ...");
                }
            }
        }else{
            ROS_ERROR("........... loop ID is not match ..............");
            ROS_ERROR("loopRequestID: %d",loopRequestID);
            ROS_ERROR("loopDetectID: %d",loopDetectID);
        }
...
void target_corner_sub(const geometry_msgs::PoseStamped::ConstPtr& msg){
    ...
    if(msg->pose.orientation.x < 0) //没有识别到目标
    {
        ...
        //记录id
        id_name = int(msg->pose.position.z);
        ...
        target_right_down.y = int(min(480,int(msg->pose.orientation.w))); //wzy 640->480
        loopDetectID = msg->pose.position.z;

        targetRegionGood = false;
        if(target_left_up.x > 100 && target_right_down.x < 540 && target_left_up.y > 80 && target_right_down.y < 400){
            targetRegionGood = true;
        }
...
bool requestVisionCb(gaofen2020::RequestVisionPoseSrv::Request  &req,
                     gaofen2020::RequestVisionPoseSrv::Response &res)
{
    isRequestFromgGaofen = req.requestVisionData;
    loopRequestID = req.num;
    res.requestVisionResult = true;
}